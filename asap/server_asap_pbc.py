...
try:
    ...
    'cannot import google.appengine.api'
...
class GetMapData(webapp2.RequestHandler):
    ...
    def get(self):
        ...
        s2now = asap_highres.get_single_asap_pbc_layer(start_date, end_date,
                                                       region=roi.geometry(), weights=weights,
                                                       add_DOY=do_add_DOY_band, add_nOBS=do_add_nOBS_band)# ***PG Edit***

        # s2now = ee.Image(ee.ImageCollection('COPERNICUS/S2').filterDate(start_date, end_date)
        #                  .filterMetadata('CLOUDY_PIXEL_PERCENTAGE', 'less_than', cloud_pcent)
        #                  .filterBounds(roi.geometry()).mosaic()).clip(roi.geometry())
        ...
        s2lastyear = asap_highres.get_single_asap_pbc_layer(start_date_1Y, end_date_1Y,
                                                            region=roi.geometry(), weights=weights,
                                                            add_DOY=do_add_DOY_band, add_nOBS=do_add_nOBS_band)  # ***PG Edit***

        # s2lastyear = ee.Image(ee.ImageCollection('COPERNICUS/S2').filterDate(start_date_1Y, end_date_1Y)
        #                       .filterMetadata('CLOUDY_PIXEL_PERCENTAGE', 'less_than', cloud_pcent)
        #                       .filterBounds(roi.geometry()).mosaic()).clip(roi.geometry())
        ...
        if False:
            # this is the old mosaic reducer based composite
            # PG EDIT
            # in order to add the DOY band for landsat, we have to break up the mosaic reducer call
            ...
            L8NowCol = ee.ImageCollection('LANDSAT/LC08/C01/T1_RT_TOA')\
                            ...
                            .filterMetadata('CLOUD_COVER_LAND', 'less_than', cloud_pcent).map(asap_highres.L8_TOA_masking).map(asap_highres.add_DOY_band).filterBounds(roi.geometry())
            ...
            # l8now = ee.Image(ee.ImageCollection('LANDSAT/LC08/C01/T1_RT_TOA').filterDate(start_date, end_date)
            #                  .filterMetadata('CLOUD_COVER_LAND', 'less_than', cloud_pcent)  # TODO TBD remove cloud threshold for L8?
            #                  .map(asap_highres.L8_TOA_masking)  # ***PG Edit***
            #                  .map(asap_highres.add_DOY_band)    # ***PG Edit***
            #                  .filterBounds(roi.geometry()).mosaic()).clip(roi.geometry())

        if True:
            l8now = asap_highres.get_single_asap_pbc_layer(start_date, end_date,sensor='L8',
                                                           region=roi.geometry(), weights=weights,
                                                           add_DOY=do_add_DOY_band, add_nOBS=do_add_nOBS_band)  # ***PG Edit***
        ...
        if False:
            ...
            L8LastCol = ee.ImageCollection('LANDSAT/LC08/C01/T1_RT_TOA').filterDate(start_date_1Y, end_date_1Y) \
                         .filterMetadata('CLOUD_COVER_LAND', 'less_than', cloud_pcent).map(asap_highres.L8_TOA_masking)\
                         .map(asap_highres.add_DOY_band).filterBounds(roi.geometry())
            ...
            # l8lastyear = ee.Image(ee.ImageCollection('LANDSAT/LC08/C01/T1_RT_TOA').filterDate(start_date_1Y, end_date_1Y)
            #                       .filterMetadata('CLOUD_COVER_LAND', 'less_than', cloud_pcent)
            #                       .map(asap_highres.L8_TOA_masking)                              # ***PG Edit***
            #                       .map(asap_highres.add_DOY_band)  # ***PG Edit***
            #                       .map(asap_highres.get_nObs_band)  # ***PG Edit***
            #                       .filterBounds(roi.geometry()).mosaic()).clip(roi.geometry())

        if True: # ***PG Edit***
            l8lastyear = asap_highres.get_single_asap_pbc_layer(start_date_1Y, end_date_1Y, sensor='L8',
                                                               region=roi.geometry(), weights=weights,
